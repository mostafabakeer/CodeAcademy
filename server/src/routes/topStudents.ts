...
export function topStudentsRoutes(store: AppStore): Router {
  const r = Router();

  // عامة للجميع — أوائل الطلبة مرتبة حسب الترتيب داخل كل صف
  r.get('/top-students', async (_req, res: Response) => {
    try {
      const items = (await store.all<any>('top:')).map(({ value }) => value);
  ...
  // ===== إدارة (أدمن) =====
  r.get('/admin/top-students', requireAuth, requireAdmin, async (_req, res: Response) => {
    try {
      const items = (await store.all<any>('top:')).map(({ value }) => value);
  ...
  r.post('/admin/top-students', requireAuth, requireAdmin, async (req, res: Response) => {
    try {
      ...
      await store.set(`top:${id}`, student);
      res.json({ student });
    } catch (e) {
      res.status(500).json({ error: (e as Error).message });
    }
  ...
  r.put('/admin/top-students/:id', requireAuth, requireAdmin, async (req, res: Response) => {
    try {
      ...
      const student = await store.get<any>(`top:${id}`);
      ...
      await store.set(`top:${id}`, updated);
