...
export function authRoutes(): Router {
  ...
  r.post('/register', async (req, res: Response) => {
    try {
      ...
      const existing = await userService.findByPhone(normPhone);
      if (existing) {
      ...
      if (isFirstUser) logger.info(`[auth] أول مستخدم سجل = أدمن (${user.fullName})`);
  ...
  r.post('/login', async (req, res: Response) => {
    try {
      ...
      const user = await userService.findByIdentifier(String(identifier));
  ...
  r.get('/me', requireAuth, async (req, res: Response) => {
    try {
      ...
      const stats = await computeStudentStats(uid);
      res.json({ user: userService.safeUser(user), stats });
    } catch (e) {
      res.status(500).json({ error: (e as Error).message });
    }
  });
