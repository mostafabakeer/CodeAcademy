...
interface ExamData {
  ...
  questions: Question[];
  lastResult?: { best?: number; score?: number; attempts?: number } | null;
}
...
interface Result {
  ...
  correct: number;
  total: number;
  review: { id: number; text: string; textEn: string; given: number | undefined; correctIndex: number; isCorrect: boolean }[];
}
...
export default function ExamTake() {
  ...
  // أدّى الامتحان من قبل ولا يسمح الأدمن بإعادته
  if (data.lastResult && !canRetake) {
    return (
      <div className="mx-auto max-w-lg">
        <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="card-fire rounded-3xl p-8 text-center">
          ...
          <h1 className="text-2xl font-black">{t('exam.takenOnce')}</h1>
          <p className="mt-3 text-sm leading-relaxed text-gray-400">{t('exam.cannotRetake')}</p>
          <div className="mt-5 grid grid-cols-2 gap-3">
            <div className="rounded-xl bg-ink-900 p-4">
              <div className="text-xs text-gray-500">{t('exam.bestScore')}</div>
              <div className="text-2xl font-black text-fire-400">{data.lastResult.best ?? data.lastResult.score ?? 0}%</div>
            </div>
            <div className="rounded-xl bg-ink-900 p-4">
          ...
          <button onClick={() => navigate('/exams')} className="btn-fire mt-6 w-full rounded-xl px-6 py-2.5 font-bold text-white">
            {t('common.back')}
          </button>
        </motion.div>
      </div>
      ...
      <div className="mx-auto max-w-3xl space-y-6">
        <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="card-fire rounded-3xl p-8 text-center">
          ...
          <div className="mt-6 flex flex-wrap justify-center gap-3">
            ...
            <button onClick={() => navigate('/exams')} className="btn-ghost-fire rounded-xl px-6 py-2.5 font-bold">
              {t('common.back')}
            </button>
          </div>
        </motion.div>
