...
async function runJavaScript(code: string): Promise<string> {
  ...
  return new Promise<string>((resolve) => {
    ...
    const timer = setTimeout(
      () =>
        finish("❌ انتهت مهلة التنفيذ (5 ثوانٍ) — تحقق من وجود حلقة لا نهائية"),
      WORKER_TIMEOUT_MS,
    );
    worker.onmessage = (e) => {
      clearTimeout(timer);
...
function resolveLocalRefs(
  ...
  const lookup = (ref: string) => {
    const clean = ref
      ...
      .toLowerCase();
    return (
      byName.get(clean) ?? byName.get(clean.split("/").pop() ?? "") ?? null
    );
  };
  ...
  out = out.replace(
    ...
    (tag, src: string) => {
      ...
      if (hit && hit.language === "javascript")
        return `<script>${hit.code}</script>`;
      return tag;
    },
  );
...
function pickHtmlForCss(
  cssName: string,
  files: { name: string; language: string; code: string }[],
): { name: string; language: string; code: string } | null {
...
export default function CodeLab() {
  ...
  const [files, setFiles] = useState<
    { id: number; name: string; language: string; updatedAt: number }[]
  >([]);
  ...
  const [fullPreviewType, setFullPreviewType] = useState<"html" | "text">(
    "html",
  );
  ...
  useEffect(() => {
    ...
    saveTimer.current = setTimeout(async () => {
      try {
        ...
        setFiles((prev) =>
          prev.map((f) =>
            f.id === current.id ? { ...f, updatedAt: d.file.updatedAt } : f,
          ),
        );
        setCurrent((prev) =>
          prev && prev.id === current.id
            ? {
                ...prev,
                versions: d.file.versions,
                updatedAt: d.file.updatedAt,
              }
            : prev,
        );
        setSaved(true);
      ...
      if (current.language === "html" || current.language === "css") {
        ...
        if (missing.length) {
          ...
          fetched
            ...
            .forEach((f) => fileCacheRef.current.set(f.id, f));
        }
        const allFiles = files
          .map((f) => fileCacheRef.current.get(f.id))
          .filter((f): f is CodeFile => !!f);
        const refs = allFiles.map((f) =>
          f.id === current.id ? { ...f, code } : f,
        );
        let result: string;
        if (current.language === "html") {
          result = resolveLocalRefs(code, refs);
        } else {
          const htmlFile = pickHtmlForCss(current.name, refs);
          result = resolveLocalRefs(
            htmlFile ? htmlFile.code : cssPreviewHtml(code),
            refs,
          );
        }
        setPreview(result);
  ...
  const openFullPreview = () => {
    if (preview !== null) {
      ...
      setFullPreviewType("html");
    } else if (output) {
      setFullPreview(output);
      setFullPreviewType("text");
    }
  };
    ...
    <div className="space-y-6">
      ...
      >
        ...
        <button
          ...
          className="btn-fire rounded-xl px-5 py-2.5 font-bold text-white"
        >
          + {t("code.newFile")}
        </button>
      </motion.div>
      ...
      <div className="grid gap-5 lg:grid-cols-[280px_1fr]">
        ...
        <motion.div
          ...
          className="card-fire rounded-2xl p-4"
        >
          ...
          {creating && (
            <div className="mb-3 space-y-2">
              ...
              <div className="flex gap-2">
                <button
                  onClick={createFile}
                  className="btn-fire flex-1 rounded-lg px-3 py-1.5 text-xs font-bold text-white"
                >
                  {t("common.add")}
                </button>
                <button
                  onClick={() => setCreating(false)}
                  className="btn-ghost-fire flex-1 rounded-lg px-3 py-1.5 text-xs font-bold"
                >
                  {t("common.cancel")}
                </button>
              </div>
          ...
          {loading ? (
            ...
            <p className="rounded-xl bg-ink-900 p-4 text-sm text-gray-500">
              {t("code.noFiles")}
            </p>
          ) : (
                ...
                <div
                  ...
                  className={`group flex cursor-pointer items-center justify-between rounded-lg px-3 py-2 transition-colors ${
                    current?.id === f.id
                      ...
                      : "bg-ink-900 hover:bg-ink-800 text-gray-300"
                  }`}
                >
        ...
        <div className="min-w-0 space-y-3">
          {current ? (
            <>
              <div className="card-fire  flex flex-wrap items-center gap-3 rounded-2xl p-4">
                ...
                {(preview !== null || output) && (
                  <button
                    ...
                    className="btn-ghost-fire rounded-lg px-4 py-2 text-sm font-bold"
                  >
                    🔍 {t("code.fullPreview")}
                  </button>
                )}
                ...
                <span
                  className={`text-xs font-bold ${saved ? "text-emerald-400" : "text-gray-600"}`}
                >
                  {saved ? `✓ ${t("code.saved")}` : "💾"}
                </span>
              </div>
              ...
              <div className="card-fire rounded-2xl p-4">
                ...
                {preview !== null ? (
                  ...
                  <pre
                    dir="ltr"
                    className="max-h-96 overflow-y-auto min-h-28 whitespace-pre-wrap rounded-xl border border-ink-600 bg-ink-900 p-4 font-mono text-sm leading-6 text-gray-200"
              ...
              {showVersions && (
                <div className="card-fire rounded-2xl p-4">
                  <h3 className="mb-3 text-sm font-bold text-gray-400">
                    {t("code.versions")}
                  </h3>
                  <div className="max-h-64 space-y-2 overflow-y-auto">
                    {[...(current.versions ?? [])].reverse().map((v, i) => (
                      <div
                        key={v.at}
                        className="flex items-center justify-between gap-3 rounded-lg bg-ink-900 px-3 py-2"
                      >
                        <span className="text-xs text-gray-400">
                          #{current.versions!.length - i} ·{" "}
                        ...
                        <button
                          ...
                          className="rounded-lg bg-fire-500/15 px-3 py-1 text-xs font-bold text-fire-300 hover:bg-fire-500/25"
                        >
                          {t("code.restore")}
                        </button>
                      </div>
            ...
            <div className="card-fire flex min-h-80 flex-col items-center justify-center rounded-2xl p-8 text-center">
              ...
              <button
                onClick={() => setCreating(true)}
                className="btn-fire mt-4 rounded-xl px-5 py-2.5 font-bold text-white"
              >
                + {t("code.newFile")}
              </button>
            </div>
        ...
        <div className="fixed inset-0 z-50 flex flex-col bg-ink-950">
          <div className="flex items-center justify-between border-b border-ink-700 bg-ink-900 px-4 py-3">
            ...
            <button
              ...
              className="rounded-lg bg-fire-500/15 px-4 py-1.5 text-sm font-bold text-fire-300 hover:bg-fire-500/25"
            >
              ✕ {t("common.close")}
            </button>
          </div>
          <div className="flex-1 overflow-hidden">
            {fullPreviewType === "html" ? (
              <iframe
                sandbox="allow-scripts  allow-modals allow-forms allow-popups"
                srcDoc={fullPreview}
                title="full preview"
              ...
              <pre
                dir="ltr"
                className="h-full overflow-y-auto bg-ink-900 p-6 font-mono text-base leading-7 text-gray-200"
                style={{ textAlign: "left" }}
              >
                {fullPreview}
              </pre>
