  ...
  return (
    <div className="space-y-12">
      ...
      <motion.section
        ...
        className="relative overflow-hidden rounded-3xl border border-fire-500/25 bg-gradient-to-br from-ink-800 via-fire-950/40 to-ink-900 px-6 py-14 text-center"
      >
        <div className="pointer-events-none absolute -top-24 start-1/2 h-72 w-72 -translate-x-1/2 rounded-full bg-fire-600/25 blur-3xl" />
        <div className="pointer-events-none absolute -bottom-32 -start-16 h-64 w-64 rounded-full bg-ember-500/20 blur-3xl" />
        <div className="pointer-events-none absolute -bottom-32 -end-16 h-64 w-64 rounded-full bg-fire-500/15 blur-3xl" />
        ...
        <div className="relative">
          <motion.div
            ...
            className="mx-auto h-36 w-36 overflow-hidden rounded-full border-4 border-fire-500/40 bg-ink-800 shadow-2xl shadow-fire-900/50 animate-flame"
          >
          ...
          <motion.p
            ...
            className="mx-auto mt-3 max-w-xl text-gray-300"
          >
            {t('top.subtitle')}
          </motion.p>
          ...
          <motion.div
            ...
            className="mt-4 inline-flex flex-col items-center"
          >
            <span className="text-xl font-black">
              {lang === 'ar' ? 'مصطفى بكير' : 'Mostafa Bakir'}
            </span>
            <span className="mt-1 rounded-full bg-fire-500/15 px-4 py-1 text-sm font-bold text-ember-400">
              👨‍🏫 {t('top.founder')} — {t('top.founderTitle')}
            </span>
          </motion.div>
          ...
          <motion.button
            ...
            onClick={share}
            className="btn-fire mx-auto mt-6 flex items-center gap-2 rounded-xl px-5 py-2.5 text-sm font-bold text-white"
          >
      ...
      {loading ? (
        ...
        <p className="rounded-2xl border border-ink-600 bg-ink-900 p-10 text-center text-gray-400">{t('top.empty')}</p>
      ) : (
        <>
          <GradeSection key="bac1" title={t('top.bac1Title')} grade="bac1" students={bac1} accent="sky" />
          <GradeSection key="bac2" title={t('top.bac2Title')} grade="bac2" students={bac2} accent="ember" />
        </>
      )}
    </div>
  );
...
function GradeSection({ title, students, accent }: { title: string; students: TopStudent[]; grade: string; accent: 'sky' | 'ember' }) {
  ...
  const header = {
    sky: { icon: '🔵', ring: 'ring-sky-500/40', grad: 'from-sky-500/15 to-transparent', badge: 'bg-sky-500/15 text-sky-300 border-sky-500/40' },
    ember: { icon: '🟠', ring: 'ring-ember-500/40', grad: 'from-ember-500/15 to-transparent', badge: 'bg-ember-500/15 text-ember-300 border-ember-500/40' },
  }[accent];

  return (
    <motion.section initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, amount: 0.15 }} transition={{ duration: 0.5 }} className="relative overflow-hidden rounded-3xl border border-ink-600 bg-ink-900/50 p-6">
      ...
      <div className="relative">
        <div className="mb-6 flex flex-col items-center gap-2 text-center">
          ...
          <span className="rounded-full border px-3 py-1 text-xs font-bold">{students.length} 🎓</span>
        </div>
...
function PodiumCol({ student, height, place }: { student: TopStudent; height: string; place: '1st' | '2nd' | '3rd' }) {
  ...
  return (
    ...
    >
      ...
      <div className="w-full truncate text-sm font-extrabold" title={student.name}>
        {student.name}
      </div>
...
function StudentCard({ s, i }: { s: TopStudent; i: number }) {
  return (
    ...
    >
      ...
      <div className="shrink-0 text-3xl">{MEDALS[s.rank] ?? `#${s.rank}`}</div>
    </motion.div>
  );
}
